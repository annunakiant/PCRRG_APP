...
# ============================================================
# BACKEND MEGABLOCK: CHECKLISTS, JOBS, PACKOUT, CONTRACTS,
# JOB FOLDER, PHOTO ROOMS, ADMIN CUSTOMIZATION
# ============================================================

# ---------- IMPORTS (ensure these exist at top of app.py) ----------
from datetime import datetime
import json
...
# DUPLICATE PackoutItem REMOVED BY MEGAFIX
    ...
    condition = db.Column(db.String(50))  # Retained / Salvageable / Non-salvageable
...
# DUPLICATE REMOVED: class AdminSettings(db.Model):
    ...
    accent_color = db.Column(db.String(20), default="#0ea5e9")
    background_color = db.Column(db.String(20), default="#020617")
    font_family = db.Column(db.String(255), default="system-ui")
...
def jobs_list():
    ...
    if q_phone:
        query = query.filter(Job.phone.ilike(f"%{q_phone}%"))
...
def delete_job(job_id):
    ...
    # Delete packout items + photos
    for item in job.packout_items:
        PackoutPhoto.query.filter_by(item_id=item.id).delete()
        db.session.delete(item)
    ...
    for room in job.rooms:
        Photo.query.filter_by(room_id=room.id).delete()
        db.session.delete(room)

    # Delete any job-level photos if you have such a relation
...
def packout_edit(job_id):
    ...
    if request.method == 'POST':
        ...
        # Photos: you can later wire actual file upload; for now, placeholder
        # if 'photos' in request.files:
        #     for f in request.files.getlist('photos'):
        #         # save file, create PackoutPhoto with URL

        db.session.commit()
        return redirect(url_for('packout_edit', job_id=job.id))

    return render_template('packout_edit.html', job=job)


@app.route('/jobs/<int:job_id>/packout/view')
@login_required
def packout_view(job_id):
    job = Job.query.get_or_404(job_id)
    # For now, packout is just job.packout_items
    return render_template('packout_view.html', job=job, packout=job)
...
def manage_contracts():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            # You can later save to static/uploads/contracts and store path
    ...
    # signed_contracts for display
    signed_contracts = SignedContract.query.order_by(SignedContract.signed_at.desc().nullslast()).all()

    return render_template('contracts.html', templates=templates, signed_contracts=signed_contracts)


@app.route('/contracts/templates/<int:template_id>/delete', methods=['POST'])
@login_required
...
def download_contract(contract_id):
    contract = SignedContract.query.get_or_404(contract_id)
    # Later: send_file or redirect to file_url
    return redirect(contract.file_url or url_for('manage_contracts'))


# ================== PHOTO ROOM GROUPING BACKEND ==================

@app.route('/jobs/<int:job_id>/rooms/add', methods=['POST'])
@login_required
def add_room(job_id):
    job = Job.query.get_or_404(job_id)
    name = request.form.get('name', '').strip()
    if name:
        room = Room(job_id=job.id, name=name)
        db.session.add(room)
        db.session.commit()
    return redirect(url_for('view_job', job_id=job.id))


@app.route('/rooms/<int:room_id>/photos')
@login_required
def view_room_photos(room_id):
    room = Room.query.get_or_404(room_id)
    return render_template('room_photos.html', room=room)


# ================== ADMIN CUSTOMIZATION BACKEND ==================
...
def admin_settings():
    ...
    if request.method == 'POST':
        settings.theme_name = request.form.get('theme_name', settings.theme_name)
