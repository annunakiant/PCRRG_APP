...
# 1) Make view_job pass task_templates into the template
with open("app.py", "r", encoding="utf-8") as f:
    code = f.read()

if "task_templates = JobTaskTemplate.query" not in code:
    code = code.replace(
...
# 2) Ensure attach_checklist route exists and uses template_id
if "/jobs/<int:job_id>/attach_checklist" not in code:
...
# 3) Fix the dropdown + form in templates/view_job.html
path = "templates/view_job.html"
with open(path, "r", encoding="utf-8") as f:
    html = f.read()

# Remove any old broken "Choose Job Type Checklist" block
...
print("✔ Checklists wired: dropdown populated + attach_checklist route working.")
